...
The mechanism variant. Open engine-turned dial, oversized spinner hub, exposed blade roots, engineering-first.
...
| | |
|---|---|
| Package | `com.xsytrance.squadron.radial.dev` |
| Version | `0.1.0-dev` (versionCode 1) |
| Dial mark | MERIDIAN / RADIAL |
| Markers | minimal |
| Horizon window | knurled |
| Dial texture | engine_turn |
| Weathering | 0% |
...
```bash
python3 tools/squadron_scaffold.py --only radial
python3 tools/generate_face.py squadron-radial
tools/build_face.sh squadron-radial
```
